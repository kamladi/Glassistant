...
import edu.cmu.cs.gabriel.GlassistantStateTracker;
...
public class VideoStreamingThread extends Thread {
	...
	private GlassistantStateTracker stateTracker;

	public VideoStreamingThread(FileDescriptor fd, String IPString, int port, Handler handler, TokenController tokenController, GlassistantStateTracker stateTracker) {
		is_running = false;
...
//					Log.d(LOG_TAG, "waiting");
		        ...
		        int step = stateTracker.getCurrentStep();
		        
				byte[] header = ("{\"id\":" + sendingFrameID + ", \"step\": \""+ Integer.toString(step) +"\" }").getBytes();
